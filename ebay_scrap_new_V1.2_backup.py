...
def scrape(item):
    try:
        ...
        '''
        try:
            seller = Soup.find('div',attrs={'class':'x-sellercard-atf__info__about-seller'}).find('span').text.strip()
        except:
        ...
        #price_element = soup.find('div', attrs={'class': 'x-price-primary'})
        #row['Price'] = price_element.find('span').text.split('$')[-1].strip() if price_element else 'Not Available'
        #row['Price'] = price_element.find('span').text.strip() if price_element else 'Not Available'

        #seller_element = soup.find('div', attrs={'class': 'x-sellercard-atf__info__about-seller'})
        #row['Seller'] = seller_element.find('span', attrs={'class': 'ux-textspans ux-textspans--BOLD'}).text.strip() if seller_element else 'Not Available'
        
        #shipping_message_element = soup.find('span', class_='x-shipping-message__textual-display')
        #shipping_message =soup.find('div',attrs={'class': 'x-shipping-message'})
        #row['shipping_message'] = shipping_message_element.text.strip()
        #'] = shipping_message.find('span',attrs={'class': 'ux-textspans'}).text.strip() if shipping_message else 'Not Available'


        #img_urls = [img.get('data-zoom-src', '') for img in soup.find('div', {'class': 'ux-image-carousel-container'}).find_all('img')[:3]]
        #row['Image URL 1'] = img_urls[0] if img_urls else ''
